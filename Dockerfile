...
ENTRYPOINT ["java", "-Dspring.profiles.active=${ENV} -POSTGRES_URL=${POSTGRES_URL} -POSTGRES_PORT=${POSTGRES_PORT} -POSTGRES_DATABASE_NAME=${POSTGRES_DATABASE_NAME} -POSTGRES_USER_NAME=${POSTGRES_USER_NAME} -POSTGRES_PASSWORD=${POSTGRES_PASSWORD}" , "-jar","/crm-vaadin-0.0.1-SNAPSHOT.jar"]
